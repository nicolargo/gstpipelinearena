...
AUDIOCAPS="application/x-rtp,media=\(string\)audio,clock-rate=\(int\)32000,encoding-name=\(string\)CELT,payload=\(int\)96,encoding-params=\(string\)1,frame-size=\(string\)480"
VIDEOPORT=5000
AUDIOPORT=5001
...
gst-launch -tv gstrtpbin name=rtpbin latency=0 buffer-mode=0 \
	...
	udpsrc caps=$AUDIOCAPS port=$AUDIOPORT do-timestamp=true \
		! rtpbin.recv_rtp_sink_1 \
			rtpbin. ! rtpceltdepay ! celtdec ! autoaudiosink
